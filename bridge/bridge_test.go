...
func TestBridge(t *testing.T) {
	...
	read := s.Render(buffer)
	if len(buffer) != read {
		t.Fatalf("could not render full buffer, got %v bytes, expected %v", read, len(buffer))
	}
	// synthState->Synth.Voices[0].Release++;
	s.Synth.Voices[0].Release++
	sbuffer := make([]float32, su_max_samples)
	read = s.Render(sbuffer)
	if len(sbuffer) != read {
		t.Fatalf("could not render second full buffer, got %v bytes, expected %v", read, len(buffer))
	}
	...
	err = binary.Write(&createdbuf, binary.BigEndian, buffer)
	if err != nil {
